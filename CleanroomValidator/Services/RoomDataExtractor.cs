...
namespace CleanroomValidator.Services
{
    public class RoomDataExtractor
    {
        ...
        {
            // 1. Try the canonical shared parameter first (set by this add-in)
            var paramService = new ParameterService();
            double sharedValue = paramService.GetRoomPressure(room);
            if (sharedValue != 0)
                return sharedValue;

            // 2. Try associated MEP space — shared param then common name variants
            var space = FindAssociatedSpace(room, _doc)
                        ?? (_linkedMechanicalDoc != null ? FindAssociatedSpaceInLinkedDoc(room) : null);

            if (space != null)
            {
                double spacePressure = paramService.GetRoomPressure(space);
                if (spacePressure != 0)
                    return spacePressure;

                // Fallback: scan common naming conventions used by other tools
                var pressureParam = space.LookupParameter("Design_Pressure")
                                    ?? space.LookupParameter("Room_Pressure")
                                    ?? space.LookupParameter("Pressure")
                                    ?? space.LookupParameter("Static Pressure")
                                    ?? space.LookupParameter("Space Pressure")
                                    ?? space.LookupParameter("Differential Pressure")
                                    ?? space.LookupParameter("Pressurization")
                                    ?? space.LookupParameter("Pressure Differential")
                                    ?? space.LookupParameter("RoomPressure")
                                    ?? space.LookupParameter("SpacePressure");

                if (pressureParam != null && pressureParam.HasValue)
                    return pressureParam.AsDouble();
            }

            // 3. Fallback: scan common name variants directly on the room element
            var roomPressureParam = room.LookupParameter("Design_Pressure")
                                    ?? room.LookupParameter("Room_Pressure")
                                    ?? room.LookupParameter("Pressure")
                                    ?? room.LookupParameter("Static Pressure")
                                    ?? room.LookupParameter("Space Pressure")
                                    ?? room.LookupParameter("Differential Pressure")
                                    ?? room.LookupParameter("Pressurization")
                                    ?? room.LookupParameter("Pressure Differential")
                                    ?? room.LookupParameter("RoomPressure");

            if (roomPressureParam != null && roomPressureParam.HasValue)
                return roomPressureParam.AsDouble();
